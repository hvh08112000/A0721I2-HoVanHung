...
select * from nhan_vien where substring_index(ho_ten, " ", -1) regexp '^[HKT]' and length(ho_ten) >= 15;

-- 3.Hiển thị thông tin của tất cả khách hàng có độ tuổi từ 18 đến 50 tuổi và có địa chỉ ở “Đà Nẵng” hoặc “Quảng Trị”.
select * from khach_hang 
where ((year(curdate()) - year(ngay_sinh)) between 18 and 50) and (dia_chi like "%Quảng Trị" or dia_chi like "%Đà Nẵng");

-- 4.Đếm xem tương ứng với mỗi khách hàng đã từng đặt phòng bao nhiêu lần. Kết quả hiển thị được sắp xếp tăng dần theo số lần đặt phòng của khách hàng. Chỉ đếm những khách hàng nào có Tên loại khách hàng là “Diamond”.
select kh.ma_khach_hang, kh.ma_loai_khach, kh.ho_ten, count(hd.ma_hop_dong) as "Số lần đặt phòng" from khach_hang kh, hop_dong hd
where kh.ma_loai_khach = 1 and kh.ma_khach_hang = hd.ma_khach_hang
...
where not exists (select * from hop_dong where ngay_lam_hop_dong between "2021-01-01" and "2021-03-31" and ma_dich_vu = dv.ma_dich_vu);
...
where year(hd.ngay_lam_hop_dong) = 2020 and not exists(select * from hop_dong where year(ngay_lam_hop_dong) = 2021 and ma_dich_vu = dv.ma_dich_vu)
...
select distinct month(ngay_lam_hop_dong) as "Tháng", count(ma_khach_hang) as "Số lượng khách hàng" from hop_dong where year(ngay_lam_hop_dong) = 2021
...
select hd.ma_hop_dong, nv.ho_ten, kh.ho_ten, kh.so_dien_thoai, dv.ten_dich_vu, sum(hdct.so_luong) as "số lượng dịch vụ đi kèm", hd.tien_dat_coc
from hop_dong hd
left join nhan_vien nv on hd.ma_nhan_vien = nv.ma_nhan_vien
left join khach_hang kh on hd.ma_khach_hang = kh.ma_khach_hang
left join dich_vu dv on hd.ma_dich_vu = dv.ma_dich_vu
left join hop_dong_chi_tiet hdct on hd.ma_hop_dong = hdct.ma_hop_dong
where (hd.ngay_lam_hop_dong between "2020-10-01" and "2020-12-31") -- and not exists(select * from hop_dong where hd.ngay_lam_hop_dong between "2021-01-01" and "2021-06-30")
group by hd.ma_hop_dong;
